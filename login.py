...
while True:
    ...
    if input_u != napss or password_p != napss:#判断是否为空文本
        if input_u == uspv1 and password_p == uspv2:
            print("欢迎！",input_u)#当登录成功时
            while True:#登录成功时操作输入
                i=input(">>>:")
        if input_u != uspv1 or password_p != uspv2:
            print("账户或密码错误，请重试！") 
    if input_u == napss or password_p == napss:
        print("账户或密码不能为空")